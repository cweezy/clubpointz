...
var SELECTORS = {
  HEADING_ROW: 'tr[bgcolor="#E0E0E0"] td',
  DATA_ROW: 'tr[bgcolor="#EEEEEE"]',
  DROPDOWN: 'select[name="ft"]',
...
var parseTeamResults = function (browser, teamData, headingData, isFirstTeam, callback) {
  if (_.isEmpty(headingData)) { 
    var row = $(browser.html()).find(SELECTORS.HEADING_ROW);
  ...
  util.parseResults(browser, raceInfo, teamData, resultKeys, SELECTORS.DATA_ROW, 200, 100, dataTransforms, saveResults);
};

var getTeamDropdown = function (browser) {
  return browser.queryAll(SELECTORS.DROPDOWN)[2];
};
    ...
    var visitTeamPage = function (i) {
      if (teamOptions[i]) {
        browser.visit(MARATHON_RESULT_URL, function () {
          ...
          browser.pressButton(SELECTORS.SUBMIT_BUTTON);
